<template>
  <div class="first-time" :style="{ 'height': height + 'px' }">
    <div style="max-width:700px;margin:auto;">

      <img id="triangleimg" src="~/assets/tketriangle.png" />

      <q-input type="password" color="white" autofocus :loading="loading" v-model="password" placeholder="Password"
        @keyup.enter="submit" class="password-input">
      </q-input>
      <br />
      <q-btn size="form-label-inverted" color="white" text-color="primary" align="center" class="q-mx-md full-width"
        @click="submit()">Sign in</q-btn>
      <br />
      <div style="margin-top:50px">
        Don't know the password? <br />Ask the Histor or <b
          style="color:white; text-decoration:underline; cursor:pointer;" @click="fakeData">continue as guest</b>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.$members.authenticate(this.password).then(data => {
        this.loading = false;
